...
{
#if UNITY_EDITOR
    public abstract class DictionaryDrawer<TK,TV> : PropertyDrawer where TK : new()
    {
        ...
        private static T DoField<T>(Rect rect, Type type, T value)
        {
            ...
            if (type.IsValueType && value is Color) 
            {
                //Debug.Log("it is color");
                //Color color = (Color)value;
                Color color = (Color)(object)value;
                return (T)(object)EditorGUI.ColorField(rect, color);
            }
        ...
        private void AddNewItem()
        {
            ...
            else if (typeof(TK).IsValueType == false) //this case is added by Maksim Fadeev so that SerializableDictionary could take any reference type as a key
            {
                key = new TK();
            }
            else if (typeof(TK).IsEnum) //this case is added by Maksim Fadeev so that DictionaryDrawer could properly add enum values;
            {
                int enumCount = Enum.GetValues(typeof(TK)).Length;
                TK emptyEnumField = (TK)Enum.ToObject(typeof(TK), enumCount++);
                key = emptyEnumField;
            }
            else 
            {
